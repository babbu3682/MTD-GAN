...
def sliding_window_inference_multi_output(
    inputs: torch.Tensor,
    ...
    **kwargs: Any,
    ...
    # Perform predictions
    seg_output_image, rec_output_image, count_map = torch.tensor(0.0, device=device), torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
    _initialized = False
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        _, seg_prob, rec_prob = predictor(window_data, *args, **kwargs)
        ...
        if not _initialized:  # init. buffer at the first iteration
            ...
            seg_output_image = torch.zeros(output_shape, dtype=torch.float32, device=device)
            rec_output_image = torch.zeros(output_shape, dtype=torch.float32, device=device)
    ...
    seg_output_image = seg_output_image / count_map
    rec_output_image = rec_output_image / count_map
