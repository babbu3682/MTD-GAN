...
class DDIM(nn.Module):
    ...
    def forward(self, x):
        with autocast(enabled=True):
            ...
            # self.scheduler.set_timesteps(num_inference_steps=30)
            output = self.inferer.sample(input_noise=noise, diffusion_model=self.diffusion_unet, scheduler=self.scheduler, conditioning=x, mode='concat', verbose=True)
...
class DPM(nn.Module):
    def __init__(self):
        ...
        self.scheduler = DPMScheduler(num_train_timesteps=1000)
...
class DPM_Multi(nn.Module):
    def __init__(self):
        super(DPM_Multi, self).__init__()
