...
# Thyroid Task
def Thyroid_Dataset_DCM(mode, patch_training):
    if mode == 'train':
...
def Thyroid_Dataset_DCM_Windowing(mode, patch_training):
    if mode == 'train':
        n_20_imgs   = list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/*Brain_3mm_DCM/Train/*/20/*/*/*.dcm')) + list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/*Brain_3mm_DCM/Valid/*/20/*/*/*.dcm'))
        n_100_imgs  = list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/*Brain_3mm_DCM/Train/*/X/*/*/*.dcm'))  + list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/*Brain_3mm_DCM/Valid/*/X/*/*/*.dcm'))
...
def TEST_Thyroid_Dataset_DCM_Windowing():
    # low_imgs  = list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/Cleansing_ThyroidCT_ArterialPhase/*/*.dcm'))
    # high_imgs = list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/Cleansing_ThyroidCT_DelayPhase/*/*.dcm'))

    low_imgs  = list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/Cleansing_ThyroidCT_DelayPhase/*/*.dcm'))
    high_imgs = list_sort_nicely(glob.glob('/workspace/sunggu/4.Dose_img2img/dataset/Cleansing_ThyroidCT_ArterialPhase/*/*.dcm'))
    ...
    transforms = Compose(
        [
            Lambdad(keys=["n_20", "n_100"], func=get_pixels_hu),
            ScaleIntensityRanged(keys=["n_20", "n_100"], a_min=-160.0, a_max=240.0, b_min=0.0, b_max=1.0, clip=True),          # No Margin
            AddChanneld(keys=["n_20", "n_100"]),         
